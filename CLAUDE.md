...
- **Member pages** (`/members/<slug>` + a `/members` index) are static SEO
  ...
  was a Python port of the tracker's JS `rollForward`; that tab is gone, so it
  is now the **only** implementation. Its option pass sweeps **every**
  disclosed option buy, not only those dated after the snapshot: an option
  bought in the days *before* the snapshot belongs to the annual report, and
  when that report's parser misses it the position exists in no path at all
  (Pelosi's Jan-2027 calls, bought 2025-12-30 against a 2025-12-31 snapshot,
  vanished from her page while AAPL survived by luck). The dedup key
  (ticker, type, strike, expiry) makes the wide sweep safe, and a disclosed
  option **sale** closes the position. **An exercise is a stock row, not an
  option row**: it is filed as `asset_type "Stock"` and still carries the
  `option` sub-object naming the contract it converted, so `_is_option_trade`
  tests `asset_type` alone — testing for the `option` key sent Pelosi's 5,000
  exercised VST shares into the option pass, which dropped them for having no
  expiry left, and it also caught a hedge-fund "Capital call of $3,723.39".
  An exercise also **closes** the contract it consumed, matched by the
  **purchase date its text names** ("purchased 1/14/25") and never by ticker +
  strike: Pelosi holds a GOOGL $150 call bought 2025-12-30 and exercised a
  GOOGL $150 call bought 2025-01-14, so closing by strike deleted the live
  one. An exercise naming no parseable date closes nothing. Options state
  **contracts x 100 = shares** where the filing gave a count (11 of 42 rows do);
  that is the disclosed position's size, **not** a valuation — an option's
  worth needs an option price we do not have, and notional exposure is not
  value. Stocks and options share **one denominator**, so an option carries a
  `pctPortfolio` like any position — excluding them showed Pelosi's $4.85M of
  contracts as 0% of her portfolio. The sum is unit-consistent because both
  `est` values are bracket midpoints; it follows that a deep-in-the-money
  contract is understated, exactly as a stock bought years ago is.
  ...
  **names its contract** ("$100 call · exp Jun 2027") rather than the company:
  ...
  snapshot date is the boundary, because a purchase on or before it is
  already inside the annual report's figure. Pelosi bought 100 BE $100 calls
  on 24 July and 100 more on 28 July; `max` reported 100 contracts worth
  $3.0M instead of 200 worth $3.75M. A total contract count is shown only
  when EVERY lot stated one. `positions` stays a STOCK count and
  `optionsCount` the option count, while `positionsTotal` (both) drives the
  headline, the ring and the "N smaller positions not listed" tail.
  Bracket midpoints, never share counts. Generated by
