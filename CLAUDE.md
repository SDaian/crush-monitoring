...
- **Member pages** (`/members/<slug>` + a `/members` index) are static SEO
  ...
  **contracts x 100 = shares** where the filing gave a count (11 of 42 rows do);
  ...
  `positions` stays a STOCK count (it drives the "N smaller positions not
  listed" tail, and every option is listed in its own block); the headline
  count and the ring's "everything else" slice add `optionsCount`.
