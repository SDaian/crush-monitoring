...
- **Member pages** (`/members/<slug>` + a `/members` index) are static SEO
  ...
  **contracts x 100 = shares** where the filing gave a count (11 of 42 rows do);
  ...
  holdings, not in a footnote. Each row carries `kind`, and an option row is
  badged so it can never read as share ownership; the detail block below
  still itemises every strike and expiry. `positions` stays a STOCK count and
