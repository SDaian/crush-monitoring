...
- **Ticker pages** (`/tickers/<slug>` + a `/tickers` index) are the organic-
  ...
  takes the top `TICKER_PAGE_COUNT` symbols clearing `TICKER_PAGE_MIN_TRADES`.
  Do NOT switch it to the featured watchlist — a featured name with 4 disclosed
  trades (NU) makes a stub, and mass thin pages are an SEO liability on a young
  domain. Generated by `landing_data.ticker_payload` + `write_ticker_files`;
  rendered by `src/pages/tickers/[slug].astro`. Cross-linked with member pages
