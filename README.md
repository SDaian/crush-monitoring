...
- `docs/trades.html` — the tracker page: featured-member watchlist,
  ...
  showing each featured member's **real individual-stock holdings** (with
  year-end value brackets) parsed from their latest **annual** disclosure
  report — falling back to an *inferred* net-trading estimate where the annual
  report is a scan — an estimated "return since buy" per disclosed buy, and a
  filterable table with every row linked to the official filing.
- `docs/data/holdings.json` — real annual-report stock holdings per featured
  member (generated by `congress/holdings.py`; individual stocks only).
